...
for(i in smp){
    model <- ml_logistic_regression(data_splitted$train, predictor, max_iter=max.iter[[i]])
    eval <- ml_evaluate(model, data_splitted$test)
...
logistic_metrics <- data.frame(i=max.iter, wp=log.precision, wr=log.recall, wf=log.fmeasure, roc=log.roc, acc=log.accuracy)
plots <- list()
...
### Validacija
```{r Validacija, message=FALSE, warning=FALSE, error=FALSE}
tbl_data <- copy_to(sc, data, "tbl_data", overwrite = TRUE)

pip <- tbl_data %>%
    ml_pipeline() %>%
    ml_logistic_regression(predictor)

evl <- ml_multiclass_classification_evaluator(x = tbl_data, metric_name = "f1")
param_grid = list(logistic_regression=list(max_iter(5, 10, 5)))
validation <- ml_cross_validator(tbl_data, pip, param_grid, evl)

cv <- ml_fit(validation, tbl_data)
metrics <- ml_validation_metrics(cv)
p <- ggplot(metrics, aes(x=max_iter, y=f1)) +
  geom_line() +
  theme_minimal()

ggsave("images/logValidation.png", p)
```
![Validacija](images/logValidation.png)
