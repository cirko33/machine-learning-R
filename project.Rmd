...
```
### Prikaz zavisnosti
```{r Prikaz zavisnosti 1, include=FALSE, message=FALSE, warning=FALSE, error=FALSE}
...
### Validacija
```{r Validacija 1, message=FALSE, warning=FALSE, error=FALSE}
pip <- sc %>% 
    ml_pipeline() %>%
...
```
### Prikaz zavisnosti
```{r Prikaz zavisnosti 2, include=FALSE, message=FALSE, warning=FALSE, error=FALSE}
...
### Validacija
```{r Validacija 2, message=FALSE, warning=FALSE, error=FALSE}
pip <- sc %>% 
    ml_pipeline() %>%
    ft_r_formula(formula) %>%
...
## Random forest
### Obučavanje i evaluacija modela
```{r Random forest, message=FALSE, warning=FALSE, error=FALSE}
smp <- c(1:3)
num.trees <- smp * 5
...
```
### Prikaz zavisnosti
```{r Prikaz zavisnosti 3, include=FALSE, message=FALSE, warning=FALSE, error=FALSE}
...
### Validacija
```{r Validacija 3, message=FALSE, warning=FALSE, error=FALSE}
pip <- sc %>% 
    ml_pipeline() %>%
    ft_r_formula(formula) %>%
...
clearFn <- function() {
    all_objects <- ls()
    needed_to_keep <- c("sc", "ds.df", "ds.filtered", "data", "conf", "clearFn", "features", "standardized_data")
    to_remove <- setdiff(all_objects, needed_to_keep)
    rm(list = to_remove)
}
```
...
## K-means
### Biranje K
```{r Biranje K, message=FALSE, eval=FALSE, warning=FALSE, error=FALSE}
clustered <- list()
for(k in 2:10) {
...
### Izračunavanje struktura klastera
```{r Izračunavanje struktura klastera, message=FALSE, warning=FALSE, error=FALSE}
kmeans.means <- list()
kmeans.medians <- list()
kmeans.counts <- list()

for(i in smp) {
    kmeans.means[[i]] <- kmeans_predictions[[i]] %>% select(-features, -payment_type) %>% group_by(prediction) %>% summarise_all(mean) %>% arrange(prediction) %>% collect()
    kmeans.medians[[i]] <- kmeans_predictions[[i]] %>% select(-features, -payment_type) %>% group_by(prediction) %>% summarise_all(median) %>% arrange(prediction) %>% collect()
    kmeans.counts[[i]] <- kmeans_predictions[[i]] %>% select(-features) %>% group_by(prediction) %>% summarise(count = n()) %>% arrange(prediction) %>% collect()
}
...
```{r Prikaz klastera, include=FALSE, message=FALSE, warning=FALSE, error=FALSE}
means <- data %>%
    summarise(total_amount=mean(total_amount)) %>%
    collect()

sds <- data %>%
    summarise(total_amount=sd(total_amount)) %>%
    collect()

for(i in smp) {
    tmp <- kmeans_predictions[[i]] %>% select(payment_type, prediction) %>% collect()
    png(paste0("images/kmeans_payment_type_", ks[[i]], ".png"), width = 800, height = 1200)
    boxplot(payment_type ~ prediction, data = tmp, main = paste0("K-means klaster sa k=", ks[[i]]), xlab = "Klaster", ylab = "Dužina putovanja")
    dev.off()
    ...
    tmp <- kmeans_predictions[[i]] %>% select(passenger_count, prediction) %>% collect()
    png(paste0("images/kmeans_passenger_count_", ks[[i]], ".png"), width = 800, height = 1200)
    boxplot(passenger_count ~ prediction, data = tmp, main = paste0("K-means klaster sa k=", ks[[i]]), xlab = "Klaster", ylab = "Napojnica")
    dev.off()
}
