...
predictor <- tip_status ~ trip_distance + fare_amount + tolls_amount + total_amount + passenger_count + payment_type
```
### Funkcija za čišćenje memorije
```{r Funkcija za čišćenje memorije, results='hide', message=FALSE, warning=FALSE, error=FALSE}
clearFn1 <- function() {
    all_objects <- ls()
    needed_to_keep <- c("sc", "ds.df", "ds.filtered", "data", "conf", "clearFn1", "split_data", "predictor")
    to_remove <- setdiff(all_objects, needed_to_keep)
    rm(list = to_remove)
}
```
...
for(i in smp){
    model <- ml_logistic_regression(split_data$train, predictor, max_iter=max.iter[[i]])
...
for(i in smp){
    model <- ml_decision_tree_classifier(split_data$train, predictor, max_depth=max.depth[[i]])
    eval <- ml_evaluate(model, split_data$test)
...
p <- decision_tree_metrics %>%
  ...
  scale_x_continuous(breaks=max.iter) +
  scale_y_continuous(breaks=log.accuracy) +
  labs(x="Maksimalna dubina stabla", y="Tacnost", title = "Zavisnost tacnosti od maksimalnog broja iteracija")
...
for(i in smp){
    model <- ml_random_forest_classifier(split_data$train, predictor, num_trees=num.trees[[i]])
    eval <- ml_evaluate(model, split_data$test)
