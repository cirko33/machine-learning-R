...
cp <- copy_to(sc, cluster_data, "cd", overwrite = TRUE)
```
### Funkcija za čišćenje memorije
```{r Funkcija za čišćenje memorije, results='hide', message=FALSE, warning=FALSE, error=FALSE}
clearFn2 <- function() {
...
clustered <- list()
for(k in 2:5) {
  clustered[[k]] <- ml_bisecting_kmeans(x=cp, formula=formula, k = k, max_iter = 10)
}
...
k_errors_df <- data.frame(i=1:5, e=k_errors)
...
```
![Sum of squared errors over number of clusters](images/kElbowPlot.png)
Pregledanjem grafika, velike promene se dešavaju na k=2 i k=4, čime su izabrane 
za parametre.

### Obučavanje
Za obučavanje su odabrani modeli sa k=2 i k=4.
```{r Obučavanje, eval=FALSE, message=FALSE, warning=FALSE, error=FALSE}
kmeans_models <- list()
kmeans_predictions <- list()
smp <- c(1:2)
for(i in smp) {
    kmeans_models[[i]] <- ml_bisecting_kmeans(x=cp, formula=formula, k = i*2)
    kmeans_predictions[[i]] <- ml_predict(kmeans_models[[i]], features=cluster_data)
}
```
### Prikaz klastera
```{r Prikaz klastera, include=FALSE, eval=FALSE, message=FALSE, warning=FALSE, error=FALSE}
p <- kmeans_predictions[[2]] %>%
        ggplot(aes(total_amount, tip_amount, color=as.factor(prediction))) +
        geom_point() +
        labs(title = paste("K-means clustering with k =", i),
             x = "Total amount",
             y = "Tip amount")
            
ggsave(paste0("images/kmeans", i, ".png"), p)

kmeans_models[[2]] <- kmeans_models[[4]]
predictions <- list()
for(i in smp) {
    kmeans_models[[i]]
...
```
![K-means klaster sa k=2](images/kmeans2.png)
![K-means klaster sa k=4](images/kmeans4.png)