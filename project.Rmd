...
```{r Analiza atributa RatecodeID, eval=TRUE, include=TRUE}
ratecodeIdCounts <- data %>%
    group_by(RatecodeID) %>% 
    summarise(count = n()) %>% 
    collect()

print("Broj vožnji po RatecodeID-u")
...
```{r Analiza atributa PULocationID, eval=TRUE, include=TRUE}
puLocationIdCounts <- data %>%
    group_by(PULocationID) %>% 
    summarise(count = n()) %>% 
    collect()

print("Broj vožnji po PULocationID-u")
...
```{r Analiza atributa DOLocationID, eval=TRUE, include=TRUE}
doLocationIdCounts <- data %>%
    group_by(DOLocationID) %>% 
    summarise(count = n()) %>% 
    collect()

print("Broj vožnji po DOLocationID-u")
...
```{r Analiza atributa payment_type, eval=TRUE, include=TRUE}
paymentTypeCounts <- data %>%
    group_by(payment_type) %>% 
    summarise(count = n()) %>% 
    collect()

print("Broj vožnji po payment_type-u")
...
dev.off()
```
![Distribucija atributa fare_amount](images/fareAmountBox.png)

### Analiza atributa extra
```{r Analiza atributa extra, eval=TRUE, include=TRUE}
extraCounts <- data %>%
    group_by(extra) %>%
    summarise(count = n()) %>%
    collect()

print("Broj vožnji po extra")
kable(extraCounts)
...
```{r Analiza atributa mta_tax, eval=TRUE, include=TRUE}
mtaTaxCounts <- data %>%
    group_by(mta_tax) %>%
    summarise(count = n()) %>%
    collect()

print("Broj vožnji po mta_tax")
kable(mtaTaxCounts)
...
bar <- ggplot(mtaTaxCounts, aes(x = mta_tax, y = count, fill = mta_tax)) +
    ...
    scale_y_continuous(breaks = seq(0, 5000000, 100000), limits=c(0, 5000000)) + 
