...
split_data <- sdf_random_split(data, training = 0.9, test = 0.1, seed = 1)
```
...
clearFn1 <- function() {
    all_objects <- ls()
    needed_to_keep <- c("sc", "ds.df", "ds.filtered", "data", "conf", "clearFn1", "split_data", "formula")
    to_remove <- setdiff(all_objects, needed_to_keep)
    rm(list = to_remove)
}
```
...
```
### Prikaz zavisnosti
```{r Prikaz zavisnosti, include=FALSE, eval=FALSE, message=FALSE, warning=FALSE, error=FALSE}
...
### Validacija
```{r Validacija, eval=FALSE, message=FALSE, warning=FALSE, error=FALSE}
pip <- sc %>% 
    ml_pipeline() %>%
...
fit <- ml_fit(x=validator, dataset=data)
...
```
### Prikaz zavisnosti
```{r Prikaz zavisnosti, eval=FALSE, include=FALSE, eval=FALSE, message=FALSE, warning=FALSE, error=FALSE}
...
### Odabir modela
Model sa najmanjim brojem stabala pokazao je najbolje rezultate (najbolju preciznost, f1, tačnost, osetljivost) prilikom testiranja
i najbolju tačnost prilikom k-fold unakrsnog validiranja sa k=4.
...
```{r Priprema podataka, message=FALSE, warning=FALSE, error=FALSE}
features <- data %>%
    select(total_amount, tip_amount, trip_distance, fare_amount)
formula <- trip_distance ~ total_amount + tip_amount + fare_amount
```
### Funkcija za čišćenje memorije
```{r Funkcija za čišćenje memorije, results='hide', message=FALSE, warning=FALSE, error=FALSE}
clearFn2 <- function() {
    all_objects <- ls()
    needed_to_keep <- c("sc", "ds.df", "ds.filtered", "data", "conf", "clearFn2", "cluster_data")
    to_remove <- setdiff(all_objects, needed_to_keep)
    rm(list = to_remove)
}
```
...
Pregledanjem grafika, velike promene se dešavaju na k=4 i k=7, čime su izabrane 
za parametre.

### Obučavanje
Za obučavanje su odabrani modeli sa k=4 i k=7.
...
for(i in smp) {
    tmp <- kmeans_predictions[[i]] %>% select(trip_distance, prediction) %>% collect()
    png(paste0("images/kmeans_trip_distance_", ks[[i]], ".png"), width = 800, height = 1200)
    boxplot(trip_distance ~ prediction, data = tmp, main = paste0("K-means klaster sa k=", ks[[i]]), xlab = "Klaster", ylab = "Dužina putovanja")
    dev.off()

    tmp <- kmeans_predictions[[i]] %>% select(total_amount, prediction) %>% collect()
    png(paste0("images/kmeans_total_amount_", ks[[i]], ".png"), width = 800, height = 1200)
    boxplot(total_amount ~ prediction, data = tmp, main = paste0("K-means klaster sa k=", ks[[i]]), xlab = "Klaster", ylab = "Potpuna cena")
    dev.off()

    tmp <- kmeans_predictions[[i]] %>% select(tip_amount, prediction) %>% collect()
    png(paste0("images/kmeans_tip_amount_", ks[[i]], ".png"), width = 800, height = 1200)
    boxplot(tip_amount ~ prediction, data = tmp, main = paste0("K-means klaster sa k=", ks[[i]]), xlab = "Klaster", ylab = "Napojnica")
    dev.off()

    tmp <- kmeans_predictions[[i]] %>% select(fare_amount, prediction) %>% collect()
    png(paste0("images/kmeans_fare_amount_", ks[[i]], ".png"), width = 800, height = 1200)
    boxplot(fare_amount ~ prediction, data = tmp, main = paste0("K-means klaster sa k=", ks[[i]]), xlab = "Klaster", ylab = "Cena vožnje")
    dev.off()
}

```
![K-means klaster sa k=4 odnos trip_distance i klastera](images/kmeans_trip_distance_4.png)
![K-means klaster sa k=4 odnos total_amount i klastera](images/kmeans_total_amount_4.png)
![K-means klaster sa k=4 odnos tip_amount i klastera](images/kmeans_tip_amount_4.png)
![K-means klaster sa k=4 odnos fare_amount i klastera](images/kmeans_fare_amount_4.png)
![K-means klaster sa k=7 odnos trip_distance i klastera](images/kmeans_trip_distance_7.png)
![K-means klaster sa k=7 odnos total_amount i klastera](images/kmeans_total_amount_7.png)
![K-means klaster sa k=7 odnos tip_amount i klastera](images/kmeans_tip_amount_7.png)
![K-means klaster sa k=7 odnos fare_amount i klastera](images/kmeans_fare_amount_7.png)
