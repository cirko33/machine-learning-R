...
## Odabir modela od svih
```{r Odabir od svih, include=FALSE, eval=FALSE, message=FALSE, warning=FALSE, error=FALSE}
metrics <- data.frame(i=max.iter, lr=log.accuracy, dt=dt.accuracy, rf=rf.accuracy)

p <- metrics %>% 
    ggplot() +
    geom_line(aes(x=i,y=lr), colour="blue") +
    geom_line(aes(x=i,y=dt), colour="red") +
    geom_line(aes(x=i,y=rf), colour="green")
    

ggsave("images/chosen.png", p)
clearFn()
```
![Odabrani model](images/chosen.png)

Odabran je model Stablo odlučivanja sa najvećom dubinom 15 zbog 
najboljih performansi u vidu tačnosti.
